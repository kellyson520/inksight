"""
全量 500+ 排版组件注册与安全度量测试 (500+ Blocks Comprehensive Test)
验证：
1. 注册表中组件总数 >= 500 (当前实测 520)
...
def test_registered_blocks_count_exceeds_500():
    """验证底层组件注册数量成功突破 500 个。"""
    count = len(BLOCK_RENDERERS)
    assert count >= 500, f"Expected at least 500 registered blocks, found {count}"


def test_all_500_blocks_can_measure_without_exception():
...
def test_all_500_blocks_can_render_without_exception():
